...
var UIController = (function(){
  ...
  return {
    ...
    setDate:function(){
      ...
      document.querySelector(DOMStrings.title).textContent = 'To Do List - ' + days[day-1] + ' ' + dayMonth + 'th' + ' ' + months[month];
...
var controller = (function(UICtrl, taskCtrl){
  ...
  var clearAll = function(){
    ...
    var delItem, getTasks;
     ...
     getTasks = taskController.getData();

     UIController.clearTasks(getTasks);
  ...
  var delTask = function(event){
    ...
    if (taskID){
      ...
      // delete from data structure
      taskController.delTaskItem(taskNo);

      UIController.delTask(taskID);

      checkData = taskController.checkDataEmp();
  ...
  var addTask = function(){
    ...
    userValues = UIController.getValues();

    if (userValues.description !== ""){
      //Description is passed into data structure to be stored
      newTask = taskController.addTaskItem(userValues.description);

      //Description is displayed onto the UI
      UIController.addTask(newTask);
